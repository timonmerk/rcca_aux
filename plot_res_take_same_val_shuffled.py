...
if GET_BEST is False:
    ...
    str_save = f"{folder_}_boxplot_comparison_best_same_shuffle_idx.pdf"
...
df_suds_c = df_.query("SUDS == True and INCLUDE_AU == False and INCLUDE_AUDIO == False and SHUFFLE == False")
idx_best_suds = df_suds_c.groupby("subject")["r"].idxmax()
df_suds_c = df_suds_c.loc[idx_best_suds]
df_suds_c["SHUFFLE"] = False
df_suds_c["condition"] = "SUDS Only"

df_suds_c_shuffle = df_.query("SUDS == True and INCLUDE_AU == False and INCLUDE_AUDIO == False and SHUFFLE == True")
rows_ = []
for sub in df_suds_c["subject"].unique():
    row = df_suds_c.query("subject == @sub")
    num_cc = row["num_cc"].values[0]
    reg = row["reg"].values[0]
    df_suds_c_sub = df_suds_c_shuffle.query("subject == @sub and num_cc == @num_cc and reg == @reg")
    rows_.append(df_suds_c_sub)
df_suds_c_shuffle = pd.concat(rows_)
df_suds_c_shuffle["SHUFFLE"] = True
df_suds_c_shuffle["condition"] = "SUDS Only"

df_all_c = df_.query("SUDS == True and INCLUDE_AU == @INCLUDE_AU and INCLUDE_AUDIO == @INCLUDE_AUDIO and SHUFFLE == False")
idx_best_all = df_all_c.groupby("subject")["r"].idxmax()
df_all_c = df_all_c.loc[idx_best_all]
df_all_c["SHUFFLE"] = False
df_all_c["condition"] = cond_name

df_all_c_shuffle = df_.query("SUDS == True and INCLUDE_AU == @INCLUDE_AU and INCLUDE_AUDIO == @INCLUDE_AUDIO and SHUFFLE == True")
rows_ = []
for sub in df_all_c["subject"].unique():
    row = df_all_c.query("subject == @sub")
    num_cc = row["num_cc"].values[0]
    reg = row["reg"].values[0]
    df_all_c_sub = df_all_c_shuffle.query("subject == @sub and num_cc == @num_cc and reg == @reg")
    
    #if sub == 11:
    #    run_sub(df_merged, sub=sub, num_cc=num_cc, reg=reg, SUDS=True, INCLUDE_AU=INCLUDE_AU, INCLUDE_AUDIO=INCLUDE_AUDIO,
    #                        NORMALIZE=True, KERNEL_CCA=False, SHUFFLE=True, out_folder_name="out2", )   
    #    df_all_c_sub = df_all_c_shuffle.query("subject == @sub and num_cc == @num_cc and reg == @reg and ktype == 'gaussian'")

    rows_.append(df_all_c_sub)
df_all_c_shuffle = pd.concat(rows_)
df_all_c_shuffle["SHUFFLE"] = True
df_all_c_shuffle["condition"] = cond_name

df_concat_c = pd.concat([df_suds_c, df_all_c, df_suds_c_shuffle, df_all_c_shuffle])
